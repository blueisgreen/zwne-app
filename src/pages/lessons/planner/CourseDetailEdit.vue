    ...
    <div class="col q-pa-sm">
      ...
      <q-card bordered>
        <q-card-section>
          ...
          />
          <q-toolbar>
            <q-toolbar-title> Lessons in Course </q-toolbar-title>
            <q-btn
          ...
          <q-list bordered separator padding class="lower-gap">
            ...
            >
            <q-item
              v-for="(lesson, index) in draftLessonPathLessons"
              :key="lesson.id"
            >
              <q-item-section>
...
const draftLessonPathLessons = computed(() => {
  if (!draftLessonPath.value) {
    return []
  }
  const lessons = draftLessonPath.value.map((id) => builder.cachedLesson(id))
  return lessons
})
